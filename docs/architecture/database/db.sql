...
CREATE TABLE topics (
  ...
  CONSTRAINT chk_difficulty CHECK (difficulty BETWEEN 0 AND 2)
);
...
DROP TABLE IF EXISTS examRooms CASCADE;
CREATE TABLE examRooms (
  exam BIGINT UNSIGNED NOT NULL REFERENCES exams(id) ON DELETE CASCADE,
  room BIGINT UNSIGNED NOT NULL REFERENCES rooms(id) ON DELETE CASCADE
);
